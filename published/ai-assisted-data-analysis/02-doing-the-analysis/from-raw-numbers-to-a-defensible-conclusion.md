...
At some point in this back-and-forth, verify a piece of the conclusion against the source data directly, rather than asking the model another question. [Why Dumping a File Rarely Works](/ai-assisted-data-analysis/01-getting-your-data-in/why-dumping-a-file-rarely-works.md) and [Code Interpreters vs. Plain Chat](/ai-assisted-data-analysis/01-getting-your-data-in/code-interpreters-vs-plain-chat.md) cover why this matters. A model's own account of its reasoning is not the same as the reasoning being correct. Every question so far in this section still runs through the model being questioned. The check that actually settles it does not. For the pricing-tier example above, pull the accounts that adopted the new tier yourself. Compare their revenue change directly against the accounts that did not, without asking the model to run that comparison for you. The model answering more questions convincingly is not the same as the underlying claim being true. A number you computed yourself, from the same rows, is.
