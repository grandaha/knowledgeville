# Making It Repeatable

This section is for a different situation than the first two: not a one-off analysis for a single meeting, but a kind of analysis you expect to run again — a recurring report, a monthly review, a check you will repeat on a new dataset. A good result on one dataset does not tell you much on its own in that situation — it could be a real method working, or it could be a prompt that happened to fit that particular file's quirks. The difference only shows up the second time, on a dataset that is not identical to the first. This section covers turning a one-off success into something you can actually rely on, and the specific test that tells you whether you have done that.

[Building a Method, Not a Lucky Prompt](/ai-assisted-data-analysis/03-making-it-repeatable/building-a-method-not-a-lucky-prompt.md)
· [Testing Your Method on a New Dataset](/ai-assisted-data-analysis/03-making-it-repeatable/testing-your-method-on-a-new-dataset.md)

## In this section

| Page | Last updated |
| --- | --- |
| [Building a Method, Not a Lucky Prompt](building-a-method-not-a-lucky-prompt.md)<br>Turning a one-off prompt that happened to work into a documented method you can run again with confidence. | Updated 2026-07-07 |
| [Testing Your Method on a New Dataset](testing-your-method-on-a-new-dataset.md)<br>Why a method that worked once needs a second, different dataset before you trust it as repeatable. | Updated 2026-07-07 |
