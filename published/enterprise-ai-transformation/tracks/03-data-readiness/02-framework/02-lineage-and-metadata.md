...
description: The evidence layer that makes data quality, governance, and access provable — four lineage types, column-level lineage, active metadata, the business glossary, model lineage, and what the "context layer" label does and does not settle.
...
sources:
  - id: ev-salesforce-state-data-analytics-2023-accessibility
    resource: "https://www.salesforce.com/en-us/wp-content/uploads/sites/4/documents/research/salesforce-state-of-data-and-analytics-2nd-edition.pdf"
    title: State of Data and Analytics (2nd Edition)
    author: Salesforce
  - id: ev-gartner-active-metadata-2025-delivery
    resource: "https://www.alation.com/blog/active-metadata/"
    title: "Market Guide for Active Metadata Management (doc ID 4004082; figure restated in later Gartner D&A research)"
    author: Gartner
...
into an auditable evidence base an agent can query at runtime.
...
For AI specifically, the glossary is where a field's meaning becomes machine-readable. An AI
agent querying `mrr_adj_enterprise` needs a definition linked to that field, not a hope that the
column name explains itself. How much a glossary improves what an agent returns has been
measured only for a narrower unit.
[Why Your AI Gives Three Different Answers](/research-notes/why-your-ai-gives-three-different-answers.md)
sets out what that does and does not establish.
...
- Known variations in definition across business units, each with its rationale and its scope
- Approved use cases for AI (and known restrictions)

That fifth field carries a trap worth naming. Record a variation only where each definition is
genuinely bounded to its own use: Finance's billed-in-period customer, Sales's
open-opportunity customer. Documenting a real boundary is useful.

It becomes an evasion when those variations are the *output* of work chartered to produce one
answer. The disagreement then survives with governance metadata attached. An agent asked a
general question still returns whichever variant the system reaches first. That outcome looks
like success in every status report, which is why it is hard to find written up as a failure.
How common it is has not been measured, and
[Why Your AI Gives Three Different Answers](/research-notes/why-your-ai-gives-three-different-answers.md)
sets out the incentive reasoning behind it.
...
## The AI Context Layer: A Category, Not Yet a Settled Pattern

Vendors increasingly market a **context layer**: lineage, metadata, and catalog unified into one
queryable graph. It holds every data asset, its meaning, its quality, its lineage, its
permissions, and its relationships.

Treat the term with care. It is a category still being defined, and largely by the firms that
sell it. Atlan's own guide to the context layer positions Atlan's product line as the delivery
mechanism for the category it is defining
([Atlan, 2026](#ev-atlan-2026-context-layer-guide-vendor-example)). For what is genuinely settled
underneath the label and what is not, see
[MCP and the Context Gap](/enterprise-ai-transformation/tracks/04-technology-architecture-and-platform/06-mcp-and-the-context-gap.md).

The underlying need is real, whatever the label. An agent working autonomously must discover
data, read what it means, verify its permissions, and judge whether an asset is trustworthy.
Those are four separate capabilities, and they can be bought or built separately.

One distinction does survive the marketing. A catalog a human reads before writing a query can
afford to be a day stale. Something an agent consults mid-task cannot.
...
| --- | --- | --- | --- |
| **Atlan** | Catalog + Active Metadata | Column-level | Positions itself as an AI context layer; strong fit for modern cloud stacks |
